...
export default function CicloColuna2() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloColuna2);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo da coluna 2" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Coluna 2 `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
