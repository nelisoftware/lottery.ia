...
export default function CicloLinha3() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloLinha3);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo da linha 3" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Linha 3 `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
