...
export default function CicloLinha2() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloLinha2);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo da linha 2" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Linha 2 `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
