...
export default function Repetindo() {
  const { data, isFetching, error } = db.Get<LotofacilNumeroSaindo[]>(Route.api.lotofacilNumerosSaindo);

  if (isFetching) {
    return <LoadingData message="Números repetindo" />
  }
    ...
    return (
      <ErrorWaring>
        <div className="flex flex-col gap-2 items-center">
          <p>Erro ao buscar números repetindo</p>
  ...
  return (
    <Card.Root className="w-full sm:w-72">
      <Card.Title title="Repetindo" icon={<Icons.tabler.Refresh />} />
      <Card.Content>
        <div>
