...
export default function CicloColuna3() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloColuna3);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo da coluna 3" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Coluna 3 `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
