...
import { LotofacilCicloDuplo, LotofacilCicloDuploNaoSaindo, LotofacilCicloTriploNaoSaindo } from "@prisma/client";
import { Card } from "../card";
import Numero from "./Numero";
import React from "react";

export default function CicloTriploNot() {
  const { data, isFetching, error } = db.Get<LotofacilCicloTriploNaoSaindo[]>(Route.api.lotofacilCicloTriploNo);

  if (isFetching) return <LoadingData message="Carregando ciclo triplo não saindo" />;
  ...
  return (
    <Card.Root className="w-full sm:w-96">
      <Card.Title title={`Ciclo Triplo - Não Saindo`} icon={<Icons.tabler.Cycle />} />
      <Card.Content>
