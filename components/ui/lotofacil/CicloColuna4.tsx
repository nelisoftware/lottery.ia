...
export default function CicloColuna4() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloColuna4);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo da coluna 4" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Coluna 4 `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
