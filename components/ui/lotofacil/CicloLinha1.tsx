...
export default function CicloLinha1() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloLinha1);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo da linha 1" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Linha 1 `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
