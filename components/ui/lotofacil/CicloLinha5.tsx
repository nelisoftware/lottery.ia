...
export default function CicloLinha5() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloLinha5);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo da linha 5" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Linha 5 `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
