...
export default function CicloPares() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloPares);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo de Pares" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Pares `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
