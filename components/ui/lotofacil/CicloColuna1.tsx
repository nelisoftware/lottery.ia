...
export default function CicloColuna1() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloColuna1);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo da coluna 1" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Coluna 1 `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
