...
export default function CicloColuna5() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloColuna5);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo da coluna 5" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Coluna 5 `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
