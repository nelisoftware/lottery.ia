...
export default function CicloImpares() {
  const { data: cycles, isFetching, error } = db.Get<LotofacilCiclo[]>(Route.api.lotofacilCicloImpares);

  const content = useMemo(() => {
    if (isFetching) return <LoadingData message="Carregando ciclo de Impares" />;
    ...
    return (
      <Card.Root className="w-full sm:w-96">
        <Card.Title title={`Ciclo Impares `} icon={<Icons.tabler.Cycle />} />
        <Card.Content>
